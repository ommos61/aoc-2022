...
int determine_score(void) {
    int empty_count = 0;
    int minx = col_count, miny = row_count, maxx = 0, maxy = 0;
    for (int y = 0; y < row_count; y++) {
        for (int x = 0; x < col_count; x++) {
            if (getpos(x, y) != '.') {
                minx = MIN(minx, x);
...
#define ROUNDS 10
    ...
    for (int round = 0; round < ROUNDS; round++) {
        ...
        if (debug) printf("Map after round %d, first_dir = %d '%c'.\n", round + 1, dir_index, dir_chars[dir_index]);
        if (debug) print_map();
    }
    int score = determine_score();
    printf("Empty ground tiles is %d.\n", score);

    printf("The answer 2447 is too low.\n");
    return EXIT_SUCCESS;
}
