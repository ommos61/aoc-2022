...
void build_distances(void) {
    for (int v = 0; v < valve_count; v++) {
        ...
        valves[v].dists_index = distance_count;
        if (strcmp(valves[v].name, "AA") != 0) valves[v].bit_index = bit_index++;
...
struct cache_item *cache[MAX_CACHE] = { NULL };

int cache_hash(int v, int t, long m) {
    return (v ^ t ^ m) % MAX_CACHE;
}

void new_cache_item(int v, int t, long m, int p) {
    ...
    new->hash = cache_hash(v, t, m);
    new->next = cache[new->hash];
    cache[new->hash] = new;
    cache_item_count += 1;
}

struct cache_item *find_item(int v, int t, long m) {
    struct cache_item *item = cache[cache_hash(v, t, m)];
    while (item != NULL) {
...
int dfs(int valve, int time, long openmask) {
    ...
    int dists_index = valves[valve].dists_index;
    struct target *targets = distances[dists_index].target_list;
    assert(targets != NULL);
    ...
    while (targets != NULL) {
        ...
        long bit = 1 << valves[target->valve].bit_index;
    ...
    new_cache_item(valve, time, openmask, maxval);
    return maxval;
}
...
int main(int argc, char *argv[]) {
    ...
    build_distances();
    print_distances();
    int start = find_valve_index("AA");
#if 0
    int total = dfs(start, 30, 0);
#else
    int total = 0;
    int mask = (1 << bit_index) - 1;
    for (int m = 0; m < (1 << bit_index) / 2; m++) {
        printf("\r%d/%d %d/%d                ", m, (mask + 1) / 2, cache_hits, cache_item_count);
        fflush(stdout);
        total = MAX(total,
                    dfs(start, 26, m) + dfs(start, 26, m ^ mask));
    }
#endif
    printf("\nTotal pressure is %d.\n", total);
    if (strcmp(fname, "input.txt") != 0) {
        printf("The example answer should be 1707.\n");
    }
