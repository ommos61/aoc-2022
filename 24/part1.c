...
struct map make_time_map(int time) {
...
state nextState(state current, state next) {
    ...
    if (next == NULL) {
        ...
        next_index = 0;
        state_count = 0;
        struct map next_map = make_time_map(current->time + 1);
...
int main(int argc, char *argv[]) {
    ...
    state e = dijkstra(&start_state, &end_state, isEndState);
    statePrint("Final state:", e);
    ...
    while (! reached_exit) {
        time += 1;
        struct map m = make_time_map(time);
