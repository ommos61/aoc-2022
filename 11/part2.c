...
#define RESULT_FALSE   "    If false: throw to monkey %d"
        ...
        if (sscanf(line, "Monkey %d:", &value) == 1) {
            ...
            // save the test divisor number (assumed it is always a prime)
...
int print_rounds[] = { 1, 20, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000, 10000};
void print_monkeys_inspected(int round) {
    static int rounds_index = 0;
    if (round == print_rounds[rounds_index]) {
...
void perform_monkey(int index, int window) {
    assert(index >= 0);
    ...
    while (cur_item != NULL) {
        ...
        switch (monkeys[index].inspect_op) {
        case OP_SQR:
            cur_item->value *= cur_item->value;
            break;
        case OP_MUL: {
            cur_item->value *= monkeys[index].inspect_val;
            break;
        };
        case OP_ADD:
            cur_item->value += monkeys[index].inspect_val;
        ...
        cur_item->value %= window;

        struct item *next_item = cur_item->next;
        cur_item->next = NULL;
        if ((cur_item->value % monkeys[index].test_divisor) == 0) {
...
void perform_round(int window) {
    for (int i = 0; i < monkey_count; i++) {
        perform_monkey(i, window);
    }
}
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < monkey_count; i++) {
        least_common_multiple *= monkeys[i].test_divisor;
    }
    printf("Least Common Multiple: %d\n", least_common_multiple);
#define ROUND_COUNT 10000
    for (int round = 1; round <= ROUND_COUNT; round++) {
        perform_round(least_common_multiple);
        print_monkeys_inspected(round);
    }
    ...
    printf("Level of monkey business is (%ld * %ld) = %ld\n", max1, max2, max1 * max2);
